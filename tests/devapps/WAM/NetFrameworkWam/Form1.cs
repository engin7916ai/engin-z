...
namespace NetDesktopWinForms
{
    ...
    {
        private IntPtr _childAppHandle;
        ...
        public Form1(IntPtr childAppHandle)
        {
            InitializeComponent();
            ...
            _childAppHandle = childAppHandle;
            Log($"My handle:{Handle.ToString()}. Child handle: {_childAppHandle.ToString()}.");
        }
        ...
        private async Task<AuthenticationResult> RunAtiAsync(IPublicClientApplication pca)
        {
            ...
            {
                builder.WithParentActivityOrWindow(Handle);
            }
            else
            {
        ...
        {
            ProcessStartInfo psi = new ProcessStartInfo(@"NetDesktopWinForms.exe", Handle.ToString());
            var childProcess = Process.Start(psi);

            //var process = Process.GetProcessesByName("notepad")[0];
            //_childAppHandle = process.MainWindowHandle;
