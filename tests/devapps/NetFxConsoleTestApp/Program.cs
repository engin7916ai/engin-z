   ...
   ""id_token"":
    {
     ...
     ""acr"": {""values"": [""urn:mace:incommon:iap:silver""] }
        ...
        private static IntPtr _childAppHandle;

#pragma warning disable UseAsyncSuffix // Use Async suffix
        public static async Task Main(string[] args)
#pragma warning restore UseAsyncSuffix // Use Async suffix
        {
            _childAppHandle = args.Length > 0 ? (IntPtr)long.Parse(args[0]) : IntPtr.Zero;
            Console.ResetColor();
        ...
        private static IPublicClientApplication CreatePca()
        {
            ...
            if (s_useBroker)
            {
                ...
                if (_childAppHandle == IntPtr.Zero)
                {
                    builder.WithParentActivityOrWindow(() => consoleWindowHandle);
                }
                else
                {
                    builder.WithParentActivityOrWindow(() => _childAppHandle);
                }
                Console.WriteLine($"My handle:{consoleWindowHandle.ToString()}. Child handle: {_childAppHandle.ToString()}.");
                builder = builder
            ...
            while (true)
            {
                ...
                try
                {
                    switch (selection)
                    {
                        ...
                        case 'f':
                            ProcessStartInfo psi = new ProcessStartInfo(@"NetFxConsoleApp.exe", GetConsoleWindow().ToString());
